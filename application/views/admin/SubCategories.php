		...
		<div class="box box-default">
			...
			<h3 class="box-title">Manage Sub Categories:- </h3>
				...
				<div class="box-tools pull-right">
					<button type="button" class="btn btn-primary addcatbtn" id="addcatbtn">Add Sub Category</button>
				...
				<div class="col-md-12">
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							...
							foreach($scategories as $index=>$value){?>
								<tr id="<?php echo "subcategroy-".$value['scatid'];?>">
									<td><?php echo $value['catname'];?></td>
									<td><?php echo $value['permalink'];?></td>
									<td><?php echo $value['description'];?></td>
									<td>
									<button data-catid="<?php echo $value['scatid'];?>" type="button" class="editCat btn btn-primary">Edit  SubCategory</button>

									<button data-catid="<?php echo $value['scatid'];?>"  type="button" class="btn deleteCat btn-danger" style="margin-top: 3px">Delete SubCategory</button>
									</td>
								</tr>
							<?php } 
							} else {?>
  ...
  <div class="modal-dialog" role="document">
   ...
   <?php } ?>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="subcatname" placeholder="Enter Name" name="name">
    </div>
    <div class="form-group">
   ...
   <div id="errorAdmin" class="alert alert-danger hide basicSetMessages"></div>
    <button type="submit" id="addCategory" class="btn btn-primary ">Add Sub Category</button>
...
<script>
			...
			buttons: {
				deleteAnswer: {
					...
					action: function () {
						...
						$.ajax({
							type:'POST',
							url:'<?php echo base_url()."post-admin-delete-subcategory";?>',
							type: "POST",
							data : fd,
							processData: false,
							contentType: false,
	...
	
	{
		...
		$.ajax({
		method:'POST',
		url:'<?php echo base_url()."post-admin-edit-subcategory";?>',
		data : fd,
		processData: false,
	    contentType: false,
		success: function(response) 
	...
	<!-- add categories-->
		...
		$.ajax({
			type:'POST',
			url:'<?php echo base_url()."post-insert-subcategories"?>',
			type: "POST",
			data : fd,
			processData: false,
			contentType: false,
	...
	<!-- update categories-->
		...
		$.ajax({
			type:'POST',
			url:'<?php echo base_url()."post-update-subcategories"?>',
			type: "POST",
			data : fd,
			processData: false,
			contentType: false,
